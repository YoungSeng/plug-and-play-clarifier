...
def get_feedback_for_object(box_xyxy: np.ndarray, image: np.ndarray, object_name: str) -> (str, bool, float):
    ...
    if is_too_large:
        return (
            f"Please move further away. The '{object_name}' is too large in the frame and may be incomplete.",
            None, None
        )
...
def load_detection_model(args, timing_recorder=None):
    ...
    if model_type == 'GROUNDINGDINO':
        ...
        if timing_recorder: timing_recorder.start("model_loading")
        model = dino_load_model(args.config_path, args.model_path, device=DEVICE)
        if args.fp16 and DEVICE == 'cuda':
            model.half()
        if timing_recorder: timing_recorder.end("model_loading")
        return model
...
def process_dataset(args, timing_recorder=None):
    """Main function to run inference over the dataset."""
    # 1. Load Model
    try:
        model = load_detection_model(args, timing_recorder=timing_recorder)
    ...
    # Excel loading
    if timing_recorder: timing_recorder.start("excel_loading")
    try:
        df = pd.read_excel(args.input_excel)
    except FileNotFoundError:
        print(f"Error: Input Excel file not found at {args.input_excel}")
        if timing_recorder: timing_recorder.end("excel_loading")
        return
    if timing_recorder: timing_recorder.end("excel_loading")
    ...
    for index, row in tqdm(df.iterrows(), total=df.shape[0], desc=f"Processing with {args.model_type}"):
        try:
            ...
            # image I/O
            if timing_recorder: timing_recorder.start("image_io")
            if not os.path.exists(image_path):
                if timing_recorder: timing_recorder.end("image_io")
            ...
            if args.model_type.upper() == 'GROUNDINGDINO':
                if timing_recorder: timing_recorder.start("detection")
                ...
                if timing_recorder: timing_recorder.end("detection")

            elif args.model_type.upper() == 'YOLOE':
                # Placeholder for YOLOE
                if timing_recorder: timing_recorder.start("detection")
            ...
            # --- Feedback Generation ---
            if timing_recorder: timing_recorder.start("feedback_generation")
            if box_xyxy is not None:
                feedback, is_blur, score = get_feedback_for_object(box_xyxy, img_bgr, object_name_en)
                ...
                feedback = f"Object '{object_name_en}' not detected in the frame."
            if timing_recorder: timing_recorder.end("feedback_generation")
    ...
    # 4. Save results
    if timing_recorder: timing_recorder.start("output_saving")
    ...
    if timing_recorder: timing_recorder.end("output_saving")
...
def main():
    ...
    process_dataset(args, timing_recorder=timing_recorder)
