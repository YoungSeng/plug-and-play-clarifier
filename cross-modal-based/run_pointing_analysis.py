...
def process_image(image_path, depth_model, pose_model, yoloe_model, yoloe_pf_model):
    """
    Processes a single image through the entire pipeline.
    This function's logic is preserved, with an update to the 'bbox' return type
    to include hand context for advanced cropping.
    """
    try:
        raw_img = cv2.imread(image_path)
        if raw_img is None: raise FileNotFoundError(f"Failed to read: {image_path}")
    except Exception as e:
        return {'success': False, 'type': 'error', 'data': str(e)}

    # --- Steps 1-5: Image Analysis ---
    try:
    ...
    # --- Final Result Processing and Decision Making ---
    final_point_info = None
    all_pf_results = None

    # Logic Branch 1: Ray Focus Mode (Legacy, preserved)
    if USE_RAY_FOCUS_INSTEAD_OF_INTERSECTION:
        # This part of the code is preserved for the 'ray focus' feature.
        # It creates a special image for the LLM by highlighting the ray's path.
        print("\n--- [Ray Focus Mode] Guiding LLM with a focus mask ---")
        ...
        if SAVE_PREPROCESSED_IMAGE_FOR_MLLM:
            ...
            print(f"   -> LLM input image saved to: {save_path}")
    ...
    # Logic Branch 2: 3D Intersection Mode (Primary path)
    else:
        ...
        if intersection_points:
            ...
            if OBJECT_MATCHING_MODE == 'specific_then_fallback':
                ...
                if matched_object:
                    ...
                    if fallback_bbox:
                        result_dict = {'success': True, 'type': 'bbox', 'data': fallback_bbox}  # Note: Legacy format
                ...
                if target_bbox:
                    # [MODIFIED] Return a dictionary with multiple BBoxes for advanced cropping
                    result_dict = {
                        'success': True,
                        'type': 'bbox',
                        'data': {
                    ...
                    # For visualization, we only care about the target BBox
    ...
    # Visualization (preserved, uses final_point_info correctly)
...
def run_excel_processing(args, depth_model, pose_model, yoloe_model, yoloe_pf_model, llm_objects):
...
def run_list_processing(args, depth_model, pose_model, yoloe_model, yoloe_pf_model, llm_objects):
